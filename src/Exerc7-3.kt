...
fun main() {
    ...
    for (turma in 0 until qntidadeTurmas) {
        ...
        var mediaAluno = IntArray(qntidadeAlunos)
        mediaAluno.forEachIndexed { index, l ->
            println("Escreva a média que o aluno ${index + 1} teve")
            mediaAluno[index] = readln().toInt()
            println("Aluno teve média $l")
...
//
//fun mAlunos (medAluno: IntArray): Double{
//    var mediaTurma: Double = 0.0
//    for (index: Int in medAluno.indices){
//        println("Escreva a média que o aluno ${index+1} teve")
//        medAluno[index] = readln().toInt()
//        mediaTurma += medAluno[index]
//    }
//    return mediaTurma
//
//}
